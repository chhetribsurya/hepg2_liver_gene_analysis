...
input_file <- "~/Dropbox/encode_3/hepg2_liver_gene_sp_analysis/files_potential_enhancer/final_tf_binding_with_gene_enh_bins_merged_barplot_data.bed"
tf_count_sp_gene_df <- fread(input_file, sep="\t", header= TRUE)

tf_count_sp_gene_df <- tf_count_sp_gene_df %>% 
						data.frame %>%
						arrange(tf_counts)
write_file = "~/Dropbox/Liver_hepg2_gene_enhancers_barplot_with_TPM_scale_data.txt"
write.table(tf_count_sp_gene_df, write_file, sep="\t", row.names=TRUE, col.names=TRUE, quote=FALSE )

output_file_name <- paste0("~/Dropbox", "/", "final_tf_count_with_enhancers_bin_barplot.pdf")				
...
barplot <- ggplot(tf_count_sp_gene_df, aes(x=reordered_gene_name, y=tf_counts)) + 
    ...
    theme_bw() + 
    ggtitle("Total TF counts across potential enhancer of each genes") + 
    theme(
    axis.text.y = element_text(size=6, face="bold", color="red" ),
...
output_file_name <- paste0("~/Dropbox", "/", "Liver_hepg2_gene_enhancers_barplot_with_TPM_scale.pdf")        
pdf(output_file_name)
# test <- data.frame(X = c("DBF", "CR/CF" ), Z = c(0.15, 0.15))
reordered_gene_name = factor(tf_count_sp_gene_df$gene_name, levels=tf_count_sp_gene_df$gene_name)
...
# Heatmap:
input_file <- "~/Dropbox/encode_3/hepg2_liver_gene_sp_analysis/files_potential_enhancer/final_tf_binding_with_gene_enh_bins_heatmap_data.bed"
...
output_file_name <- paste0("~/Dropbox", "/", "final_tf_count_with_enhancers_bin_heatmap.pdf")				
pdf(output_file_name)
...
test <- heatmap.2(mat_data,
  ...
  # Colv = TRUE,
  #scale="none",
  main = "TF occupancy across potential enhancer of 52 Liver-HepG2 specific/expressed genes", 
  xlab = "Gene names",
...
output_file_name <- paste0("~/Dropbox", "/", "Liver_hepg2_enhancers_heatmap_TPM_chart_1.pdf")       
pdf(output_file_name)
...
# dist(mat_data)
# data        = data.matrix(mat_data)
# distance    = dist(mat_data)
# cluster     = hclust(distance, method="ward")
# dendrogram  = as.dendrogram(cluster)
# Rowv        = rowMeans(mat_data, na.rm = T)
