...
#####################
# 5C data for the HepG2:

# file_dir = os.path.expanduser("~/Dropbox/encode_downloads/5c_data/wgEncodeUw5CHepg2Uw5cgeneMatrixRep2")
# file_list = glob.glob(join(file_dir, "*matrix.txt"))

# concat_df_list = []
# for each_file in file_list:
# 	read_df = pd.read_csv(each_file, sep="\t")
# 	reset_df = read_df.iloc[:,0].reset_index().loc[:, "index"]
# 	reset_df["splitted"] = [re.split("[-|:]", each) for each in reset_df]
# 	#reset_df["splitted"].apply(pd.Series)
# 	header = ["cell_line","feat_1", "feat_2", "feat_3", "genome_assembly", "chrom", "start", "end"]
# 	zipped_list = zip(*reset_df["splitted"])
# 	df_dict = dict(zip(header,zipped_list))
# 	final_df = pd.DataFrame(df_dict)
# 	concat_df_list.append(final_df)

# df = pd.concat(concat_df_list, ignore_index=True)
# df_select = df[df["cell_line"] == "HepG2"]
# df_select["chrom"].unique()
